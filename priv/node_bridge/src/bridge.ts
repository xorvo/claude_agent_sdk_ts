...
import { query, type SDKMessage, type Options, type Query } from "@anthropic-ai/claude-agent-sdk";
...
// Types for communication with Elixir
...
interface ToolResultCommand extends BaseCommand {
  type: "tool_result";
  toolUseId: string;
  result: unknown;
}

type Command = ChatCommand | ToolResultCommand;

interface Response {
  id: string;
  type: "chunk" | "complete" | "error" | "tool_use" | "end" | "system" | "result";
  content?: string;
  message?: string;
  name?: string;
...
function extractToolUse(
  message: SDKMessage
): { name: string; input: Record<string, unknown>; id: string } | null {
  if (message.type === "assistant" && message.message && "content" in message.message) {
    const content = message.message.content;
    if (Array.isArray(content)) {
      const toolUse = content.find(
        (block): block is { type: "tool_use"; name: string; input: Record<string, unknown>; id: string } =>
          block.type === "tool_use"
      );
      if (toolUse) {
...
function buildOptions(opts: ChatCommand["options"] = {}): Options {
  const sdkOptions: Options = {};

  if (opts.model) sdkOptions.model = opts.model;
  if (opts.maxTurns) sdkOptions.maxTurns = opts.maxTurns;
  if (opts.maxBudgetUsd) sdkOptions.maxBudgetUsd = opts.maxBudgetUsd;
  if (opts.cwd) sdkOptions.cwd = opts.cwd;
  if (opts.allowedTools) sdkOptions.allowedTools = opts.allowedTools;
  if (opts.disallowedTools) sdkOptions.disallowedTools = opts.disallowedTools;
  if (opts.permissionMode) sdkOptions.permissionMode = opts.permissionMode;
  if (opts.systemPrompt) sdkOptions.systemPrompt = opts.systemPrompt;
...
async function handleChat(command: ChatCommand): Promise<void> {
  ...
  try {
    ...
    debug("Calling query()...");
    const response: Query = query({ prompt, options: sdkOptions });
...
async function handleStream(command: ChatCommand): Promise<void> {
  const { id, prompt, options = {} } = command;
  ...
  rl.on("line", async (line: string) => {
    ...
    switch (command.type) {
      case "chat":
        ...
        await handleChat(command as ChatCommand);
        break;
      case "stream":
        debug("Dispatching to handleStream");
        await handleStream(command as ChatCommand);
        break;
      case "tool_result":
        send({
          id: command.id,
      ...
      default: {
        ...
        const unknownCmd = command as BaseCommand;
        send({
